...
**Конструктор сущностей** позволяет каждой компании самостоятельно создавать произвольные типы объектов (например, «Клиенты», «Сделки», «Задачи»), определять поля нужных типов (`text`, `number`, `date`, `boolean`, `select`, `email`, `phone`, `url` и др.) и немедленно работать с записями. Данные полей хранятся в колонке типа JSONB, ключи соответствуют `slug`-идентификаторам полей.
...
├── nginx/
│   └── nginx.prod.conf         # Шаблон конфигурации хостового nginx (SSL, проксирование)
├── docker-compose.dev.yml      # Dev-окружение (с hot-reload)
├── docker-compose.prod.yml     # Prod-окружение (сервер с существующим nginx)
